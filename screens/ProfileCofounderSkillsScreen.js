...
const COFOUNDER_SKILLS_ENDPOINT = '/api/v1/cofounder-skills';

function buildCofounderSkillsEndpoint(base) {
  const normalized = String(base || '').replace(/\/$/, '');
  if (!normalized) {
    return `http://127.0.0.1:8000${COFOUNDER_SKILLS_ENDPOINT}`;
  }

  if (normalized.endsWith('/api/v1')) {
    return `${normalized}/cofounder-skills`;
  }

  return `${normalized}${COFOUNDER_SKILLS_ENDPOINT}`;
}
...
function normalizeInitialSkills(initialCofounderSkills) {
  if (Array.isArray(initialCofounderSkills)) {
    return initialCofounderSkills
      .map((value) => {
  ...
  const single = typeof initialCofounderSkills === 'number'
    ? initialCofounderSkills
    : String(initialCofounderSkills || '').trim();
...
export default function ProfileCofounderSkillsScreen({
  onBack,
  onContinue,
  initialCofounderSkills = [],
}) {
  ...
  const [selectedSkills, setSelectedSkills] = useState(() => normalizeInitialSkills(initialCofounderSkills));
  ...
  React.useEffect(() => {
    ...
    const fetchCofounderSkills = async () => {
      try {
        setLoading(true);
        setErrorMessage('');

        const endpoint = buildCofounderSkillsEndpoint(process.env.EXPO_PUBLIC_API_BASE_URL || BASE_URL);
        ...
        if (!response.ok) {
          throw new Error(`Failed to load cofounder skills: ${response.status}`);
        }
        ...
        if (isMounted) {
          setErrorMessage('Could not load cofounder skills. Please try again.');
        }
      } finally {
        if (isMounted) {
          setLoading(false);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        >
          ...
          <Text style={styles.heading}>Cofounder's Skills</Text>
        ...
        <Pressable
          ...
          onPress={() => onContinue?.({ cofounderSkills: selectedSkills })}
        >
