...
function toRoleModel(item) {
  const roleName = String(item?.role_name || item?.roleName || item?.label || '').trim();

  return {
    value: item?.id ?? roleName,
    roleName,
    equityOffer: String(item?.equity_offer || item?.equityOffer || '').trim(),
    description: String(item?.description || '').trim(),
    icon: resolveRoleIcon(item?.icon),
    selectedIcon: resolveRoleIcon(item?.selected_icon || item?.selectedIcon),
  };
}

function RoleCard({ option, selected, onPress, styles }) {
...
export default function ProfileUserRoleScreen({ onBack, onContinue, initialUserRole = '' }) {
  ...
  const canContinue = Boolean(userRole);
      ...
      try {
        ...
        if (isMounted) {
          const selectedOption = mapped.find((item) => (
            item.value === initialUserRole || item.roleName === initialUserRole
          ));
        ...
        if (isMounted) {
          setErrorMessage('Could not load user roles. Please try again.');
        }
      } finally {
        if (isMounted) {
          setLoading(false);
          ...
          {loading ? (
            ...
            roleOptions.map((option) => (
              <RoleCard
                key={option.value}
                option={option}
                selected={userRole === option.value}
